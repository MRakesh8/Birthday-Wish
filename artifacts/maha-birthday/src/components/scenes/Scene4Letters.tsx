...
export function Scene4Letters({ onComplete }: { onComplete: () => void }) {
  ...
  return (
    ...
    >
      ...
      >
        <h2 className="font-serif text-2xl sm:text-3xl text-pink-200 glow-text mb-2 tracking-wide">
          Letters of My Heart
        </h2>
        <p className="font-sans text-xs sm:text-sm text-white/70 italic flex items-center justify-center gap-2">
      ...
      <div className="grid grid-cols-2 gap-4 max-w-sm sm:max-w-md w-full px-1">
        {cards.map((card, i) => (
          ...
          >
            ...
            >
              {/* Front Side */}
              <div className="absolute inset-0 w-full h-full backface-hidden glass-card rounded-2xl p-4 flex flex-col items-center justify-between border-2 border-amber-400/40 shadow-xl overflow-hidden">
                {/* Decorative inner border */}
                <div className="absolute inset-2 border border-dashed border-amber-300/25 rounded-xl pointer-events-none" />
                
                <div className="text-amber-400/50 text-[10px] sm:text-xs font-serif tracking-widest mt-1">
                  ✦ LETTER {i + 1} ✦
                </div>

                <div className="text-6xl sm:text-7xl font-script text-white glow-gold select-none transform transition-transform duration-300 group-hover:scale-110">
                  {card.letter}
                </div>

                <div className="flex items-center gap-1 text-[11px] sm:text-xs text-pink-300/80 font-serif italic mb-1 animate-pulse">
